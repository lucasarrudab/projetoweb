    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
        {products.map((produto) => (
          <div key={produto.id} className="bg-white rounded-lg shadow-md overflow-hidden relative">
            {produto.amount <= 15 && isAdmin && (
              <div className="absolute top-2 right-2 bg-red-100 text-red-600 px-2 py-1 rounded-full flex items-center text-sm">
                <ExclamationCircleIcon className="h-4 w-4 mr-1" />
                Estoque Baixo: {produto.amount}
              </div>
            )}
            ...
            <div className="p-4">
              ...
              <p className="mt-2 text-lg font-bold text-gray-900">R$ {Number(produto.price).toFixed(2)}</p>
              {isAdmin && (
                <p className="mt-1 text-sm text-gray-600">Estoque: {produto.amount}</p>
              )}
